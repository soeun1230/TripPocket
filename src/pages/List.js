...
class List extends Component {
  ...
  handleCalenderClick = (calendarUid) => {
    this.setState({ selectedCalendarUid: calendarUid });
    const userId = `user_db/${this.state.user.uid}`;
    const database = getDatabase(firebase);
    const databaseRef = ref(
      database,
      `${userId}/calendar_numbers/${calendarUid}`
    );

    onValue(databaseRef, (snapshot) => {
      const calendarData = snapshot.val();
      if (calendarData) {
        console.log("선택한 캘린더의 데이터:", calendarData);
        // calendarData를 사용하여 선택한 캘린더의 데이터를 처리
      } else {
        console.log("캘린더 데이터를 찾을 수 없음");
      }
    });
  };
